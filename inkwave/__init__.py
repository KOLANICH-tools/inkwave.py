...
MODE = IntEnum("MODE", {el.name.upper(): el.value for el in EinkWbf.Header.Mode})
...
fpl_rates = {
	EinkWbf.Header.FplRate.r_50_hz: "50Hz",
	EinkWbf.Header.FplRate.r_60_hz: "60Hz",
	EinkWbf.Header.FplRate.r_85_hz: "85Hz",
}
...
class waveform_data_header:
	...
	def __bytes__(self):
		return self.__class__.parser.pack((self.checksum, self.size, self.serial, self.run_type, self.fpl_platform, self.fpl_lot, self.mode_version_or_adhesive_run_num, self.waveform_version, self.waveform_subversion, self.waveform_type, self.fpl_size, self.mfg_code, self.waveform_tuning_bias_or_rev, self.fpl_rate, self.unknown0, self.vcom_shifted, self.unknown1, self.xwia.lo, self.xwia.hi, self.cs1, self.waveform_modes_table.lo, self.waveform_modes_table.hi, self.fvsn, self.luts, self.mode_count, self.temperature_range_count, self.advanced_wfm_flags, self.eb, self.sb, self.reserved_or_unkn, self.reserved0_2, self.reserved0_3, self.reserved0_4, self.reserved0_5, self.cs2))
...
def compare_checksum(data: str, header: waveform_data_header) -> int:
	if crc32(data[4 : header.size], CRC32_START_VALUE) != header.checksum:
		return -1
	return 0
...
def print_header(header: waveform_data_header, is_wbf: int) -> None:
	...
	print("	Run type: " + hex(header.run_type.value) + " | " + get_desc(run_types, header.run_type, "Unknown"))
	print("	Manufacturer code: " + hex(header.mfg_code.value) + " | " + get_desc_mfg_code(header.mfg_code))

	print("	Frontplane Laminate (FPL) platform: " + hex(header.fpl_platform.value) + " | " + get_desc(fpl_platforms, header.fpl_platform, "Unknown"))
	print("	Frontplane Laminate (FPL) lot: " + str(header.fpl_lot))
	print("	Frontplane Laminate (FPL) size: " + hex(header.fpl_size.value) + " | " + get_desc(fpl_sizes, header.fpl_size, "Unknown"))
	print("	Frontplane Laminate (FPL) rate: " + hex(header.fpl_rate.value) + " | " + get_desc(fpl_rates, header.fpl_rate, "Unknown"))
	...
	if isinstance(header.waveform_type, EinkWbf.Header.WaveformType):
		...
		waveform_type_int_repr = header.waveform_type.value
	else:
		waveform_type_text_repr = "Unknown"
		waveform_type_int_repr = header.waveform_type

	print("	Waveform type: " + hex(waveform_type_int_repr) + " | " + waveform_type_text_repr)
